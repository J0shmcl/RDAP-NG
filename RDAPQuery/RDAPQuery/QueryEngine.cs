...
namespace RDAPQuery
{
    ...
    public class QueryEngine
    {
        ...
        {
            _logger.LogInformation($"CheckDomains Timer trigger function executed at: {DateTime.Now}");
            ...
            try
            {
                ...
                _logger.LogInformation(string.Format("Calling QueryData with query '{0}'", queryBody));
                Task<QueryResults> task = LogAnalytics.QueryData(queryBody);
                QueryResults results = task.Result;
                _logger.LogInformation(string.Format("Retrieved: {0} rows.  Beginning resolution with Bootstrap lookups", results.tables[0].rows.Count));
            ...
            _logger.LogInformation($"CheckDomains Timer trigger function completed at: {DateTime.Now}");
        }
